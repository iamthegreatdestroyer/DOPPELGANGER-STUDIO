...
class TestJokeOptimizer:
    ...
    async def test_optimize_script_comedy_success(
        ...
        scene_dialogue = SceneDialogue(
            scene_number=1,
            location="Spaceship Hold",
            characters_present=["Lucy"],
            dialogue_lines=[
                DialogueLine(
                    character="Lucy",
    ...
    async def test_optimize_script_comedy_with_failure(
        ...
        # Need to provide a scene and beat to trigger the AI calls
        scene_dialogue = SceneDialogue(
            scene_number=1,
            location="Test",
            characters_present=["Luna"],
            dialogue_lines=[DialogueLine("Luna", "Test", "happy", "", False)],
        ...
        # With fallback, we get 1 joke with basic structure
        assert len(result.analyzed_jokes) == 1
        assert result.analyzed_jokes[0].effectiveness_score == 0.5  # Fallback score
        assert "AI analysis unavailable" in result.analyzed_jokes[0].improvement_suggestions
        # Overall effectiveness is average of all jokes (0.5 for the one fallback joke)
        assert result.overall_effectiveness == 0.5
        # Confidence is still 0.85 because optimization completed (with fallbacks)
    ...
    def test_analyze_comedy_timing_slow_burn(self, joke_optimizer):
        ...
        analyzed_jokes = [
            JokeStructure(
                ...
                setup="Setup",
                punchline="Punchline",
                timing_position=i * 150.0,  # Every 2.5 minutes (150s > 120s threshold)
                characters_involved=["Lucy"],
                effectiveness_score=0.80
            )
            for i in range(5)
        ...
        assert result.average_spacing == 150.0
        assert len(result.dead_zones) > 0  # Should detect dead zones (spacing > 120)
