...
def mock_generator_with_timing():
    ...
    with patch('src.services.creative.script_generator.ClaudeClient'), \
        ...
        async def mock_generate_dialogue(*args, **kwargs):
            ...
            from src.services.creative.character_voice_profiles import SceneDialogue, DialogueLine
        ...
        def mock_optimize_comedy(*args, **kwargs):
            from src.services.creative.joke_models import ComedyAnalysis, TimingAnalysis
            return ComedyAnalysis(
                timing_analysis=TimingAnalysis(
                    total_jokes=5,
                    jokes_per_minute=2.0,
                    average_setup_time=5.0,
    ...
    async def test_parallel_vs_sequential_speedup(
        ...
        with patch('src.services.creative.script_generator.ClaudeClient'), \
            ...
            async def mock_dialogue(*args, **kwargs):
                ...
                return SceneDialogue(
                    scene_number=1,
                    dialogue_lines=[DialogueLine("Test", "line", "neutral", "normal", "normal")]
                )
            ...
            for gen in [sequential_generator, parallel_generator]:
                ...
                from src.services.creative.joke_models import ComedyAnalysis, TimingAnalysis
                gen.joke_optimizer.optimize_script_comedy = Mock(return_value=ComedyAnalysis(
                    timing_analysis=TimingAnalysis(5, 2.0, 5.0, 2.0, 0.85),
                    analyzed_jokes=[],
                    overall_effectiveness=0.80,
                    improvement_suggestions=[],
                    character_comedy_balance={}
...
class TestBottleneckDetection:
    ...
    async def test_bottleneck_detection_identifies_slow_operations(
        ...
        with patch('src.services.creative.script_generator.ClaudeClient'), \
            ...
            from src.services.creative.joke_models import ComedyAnalysis, TimingAnalysis
            generator.joke_optimizer.optimize_script_comedy = Mock(return_value=ComedyAnalysis(
                timing_analysis=TimingAnalysis(5, 2.0, 5.0, 2.0, 0.85),
                analyzed_jokes=[],
                overall_effectiveness=0.80,
                improvement_suggestions=[],
