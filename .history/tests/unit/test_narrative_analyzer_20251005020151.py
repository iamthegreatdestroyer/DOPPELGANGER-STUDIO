...
class TestNarrativeAnalyzerFallback:
    ...
    async def test_claude_to_gpt_fallback(
        ...
        assert result is not None
        assert mock_claude_client.generate.call_count == 3  # Claude exhausted
        assert mock_gpt_client.generate.called  # GPT used
...
class TestNarrativeAnalyzerCaching:
    ...
    async def test_cache_hit(
        ...
        mock_database.mongodb['ai_analysis'].find_one.return_value = cached_data
...
class TestNarrativeDataStructures:
    ...
    def test_episode_structure_creation(self):
        ...
        assert structure.runtime_minutes == 22
